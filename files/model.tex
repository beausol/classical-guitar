...
\end{equation}
giving $\kappa \approx 49$ and $\sqrt{\kappa} \approx 7$, These are the values listed in \tbl{string_specs}, and the corresponding open-string bending stiffness is computed using \eqn{b_0_kappa} using the scale length given in \tbl{mcg_specs}. In \fig{uncomp}, we plot the total frequency shift given by \eqn{error_def} for this string on the Classical Guitar with $\Delta S = \Delta N = 0$~mm. Note that the string is sharp at every fret, but even a large nonzero value of $d$ is only important at the first fret.
