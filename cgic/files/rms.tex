...
\begin{subequations}
  ...
  \end{align}
\end{subequations}
In \sct{model}, we determined that --- for the purposes of estimating the values of the setbacks --- that $W_n$ could be represented accurately by
\begin{equation}
  W_n \approx \frac{\Delta N - \left(\gamma_n - 1\right) \Delta S}{X_0}\, .
\end{equation}
Furthermore, we discovered that $Z_n$ does not depend even to second order on either $\Delta S$ or $\Delta N$. 

Therefore, the components of the gradient of $\chi^2$ are
\begin{subequations}
  \begin{align}
