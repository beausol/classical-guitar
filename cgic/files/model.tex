...
Let's guide our intuition by relying on Taylor series expansions for all the effects described above. First, we'll ignore all quadratic terms in the resonant length error and adopt \eqn{rle_approx}. Next, we'll neglect the small reduction in linear mass density caused by fretting, and then rely on the approximation to the frequency shift caused by tension increases given by \eqn{tension_shift}. Finally, we'll describe the effects of bending stiffness using \eqn{dnu_bn}, \emph{neglecting} the term proportional to $B_0^2$. Incorporating all of these effects, we find that the total frequency shift is given approximately by
