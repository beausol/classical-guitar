...
\delta T_n = \mathcal{A}\, E_\mathrm{eff}\, Q_n\, ,
 \end{equation}
where $Q_n$ is the normalized infinitesimal displacement --- here acting as the differential strain --- defined by \eqn{q_n_def}. Note that we are using the length $L_0$ of the unfretted string between the saddle and the nut as our reference length. In the case of steel strings on electric guitars, it is appropriate to include the length of the string between the nut and the tuning peg because the string can slide freely through the nut and the tension on either side is the same~\cite{ref:kemp2020ibg}. But a nylon string on a classical guitar emerges from the outside of the nut at a sharp angle, and is held so tightly within the nut that the tension of the string between the nut and the tuners is significantly higher than that of the vibrating string by an amount that depends on the string and the manufacturing choices made for the nut.
